...
class Ag:
    ...
    def act(self, p, e):
        ...
        for d in [(0,1),(1,0),(0,-1),(-1,0)]:
            nx,ny = self.x+d[0],self.y+d[1]
            if 0<=nx<4 and 0<=ny<4 and self.k[nx][ny]=='S' and (nx,ny) not in self.visited:
                self.x,self.y = nx,ny
        ...
        for d in [(0,1),(1,0),(0,-1),(-1,0)]:
            nx,ny = self.x+d[0],self.y+d[1]
            if 0<=nx<4 and 0<=ny<4 and self.k[nx][ny]=='U' and (nx,ny) not in self.visited:
                self.x,self.y = nx,ny
                self.visited.add((nx,ny))
                self.t.append('Move '+str((nx,ny)))
                return False
        for d in [(0,1),(1,0),(0,-1),(-1,0)]:
            nx,ny = self.x+d[0],self.y+d[1]
            if 0<=nx<4 and 0<=ny<4 and self.k[nx][ny]=='P?' and (nx,ny) not in self.visited:
                self.x,self.y = nx,ny
                self.visited.add((nx,ny))
                self.t.append('Move (risk) '+str((nx,ny)))
                return False
        self.t.append('Stuck')
        return 'STUCK'
    def ret(self):
        if self.g and (self.x,self.y)==(0,0):
            ...
            return True
        if self.g:
            # Simple return to (0,0)
            if self.x > 0:
                self.x -= 1
                self.t.append('Move '+str((self.x,self.y)))
            elif self.y > 0:
                self.y -= 1
                self.t.append('Move '+str((self.x,self.y)))
            else:
...
def sim(g, w, p):
    ...
    while True:
        ...
        if res == True:
            ...
            print('Action:',a.t[-1])
            print('Final Result: FAIL (Agent stuck)')
            break
        else:
