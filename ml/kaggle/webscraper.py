...
col = ['Index', 'Name', 'CMP', 'PE', 'Market Cap', 'Dividend Yield', 'Net Profit Last Quarter', 
       'Quaterly Profit Growth', 'Sales Latest Quarter', 'Quaterly Sales Growth', 'Return on Campital Employment']
...
for i in range(1, 88):
    print(i)
    if i < 10:
        url = f"{BASE_URL}0000000{i}/?page=1"
    else:
        url = f"{BASE_URL}000000{i}/?page=1"
    ...
    for j in range(1, nump + 1):
        if i < 10:
            url = f"{BASE_URL}0000000{i}/?page={j}"
        else:
            url = f"{BASE_URL}000000{i}/?page={j}"
